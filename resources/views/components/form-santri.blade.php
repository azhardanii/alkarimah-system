            ...
            <div class="mt-5 md:mt-0 md:col-span-2">
                ...
                <div class="grid grid-cols-6 gap-6 mt-7">
                    ...
                    <div class="col-span-6 sm:col-span-3 lg:col-span-2">
                        ...
                        <select id="jenjang_formal" name="jenjang_formal" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm @error('jenjang_formal') text-red-700 @enderror">
                            <option value="" disabled selected> </option>
                    ...
                    <div class="col-span-6 sm:col-span-3 lg:col-span-2">
                        ...
                        <select id="status_pondok" name="status_pondok" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm @error('status') text-red-700 @enderror">
                            ...
                            <option value="Pindah" {{ old('status_pondok', $santri->status_pondok ?? '') == 'Pindah' ? 'selected' : '' }}>Pindah</option>
                            <option value="Alumni" {{ old('status_pondok', $santri->status_pondok ?? '') == 'Alumni' ? 'selected' : '' }}>Alumni</option>
                            <option value="Drop Out" {{ old('status_pondok', $santri->status_pondok ?? '') == 'Drop Out' ? 'selected' : '' }}>Drop Out</option>
                        </select>
                        @error('status_pondok')
                            <div class="text-red-700 text-sm">{{ $message }}</div>
                        @enderror
...
<script>
    ...
    dropArea.addEventListener('drop', (event) => {
        ...
        if (files.length > 0) {
            fotoInput.files = files;  // Set the file input's files to the dropped files
            previewImage(files[0]);  // Preview the first dropped file
        }
    });
</script>